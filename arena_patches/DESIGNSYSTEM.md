...
| P3 | Fassungen als Assets: 9-Slice-Panel-Platte, Kartenrahmen je Rarität, Bannerplatte | **erledigt** (§39.1). Kartenrahmen und Bänder seit §6c; Panel-Platte und `progress_frame` am 31.07.2026 nachgezogen. `panel.webp` lag mit `background-size:auto` auf `.reqpanel` — eine 1200×896-Grafik in einem 406er Kasten zeigt ihr linkes oberes Drittel, also einen Rahmen ohne zwei Seiten. Zuschnitt aus den Pixeln gemessen (54 px Rand + 54 px Eckzier = 108 px). `progress_frame` ist waagerecht und wurde lokal auf die Balken-Bounding-Box beschnitten (`progress_frame_cut`, in `HERKUNFT.json` vermerkt) |
| P4 | Bewegungssprache: Overshoot, Squash, Hochzählen, Belohnungs-Choreografie | **erledigt** (§39.4). Overshoot, Hochzählen (`zaehlHoch`) und die gestaffelte Belohnungs-Choreografie (70 ms, synchron zum Ton) standen bereits; **Squash & Stretch** war das fehlende Stück und ist am 31.07.2026 nachgezogen — nicht-gleichförmige Skalierung bei `curtreffer` und `belpop`, Achsensumme nahe 2 |
| — | Typo-Leiter | **erledigt, mit benanntem Rest** (§39.2): 31 Größen → die sechs halben Stufen sind in die ganze darüber gewandert, neun Stufen sind benannt und decken 504 der 682 Verwendungen. Gemessen gegen 1 529 Textelemente: 0 neue Überläufe, 2 echte Funde behoben. Die verbleibenden 178 Literale sind Einzelfälle (Kartentitel, Zeremonie-Schrift, das 96-px-Siegel) — sie auf eine Stufe zu zwingen wäre Aufräumen gegen die Sache |
| — | Radien | **erledigt** (§39.1): 17 Werte in 267 Verwendungen auf sechs benannte Stufen, gebaut um die beiden Tokens herum, die bereits 102-mal in Gebrauch waren. Der tote Zweitsatz `--r-1..4` ist weg |
| — | Fehlende Komponenten | **Gestaltete Scrollbars** erledigt (§39.6), **Ladezustände** seit §35. **Tooltips bewusst nicht gebaut**: ein Tooltip hängt am Zeigen, auf dem Telefon gibt es kein Zeigen — das Muster für dieselbe Aufgabe ist das ⓘ zum Antippen, an vier Stellen gebaut. **Health/Mana-Balken, Damage-Zahlen, Kampagne** gehören in den Kampfbildschirm und damit nicht in diese Datei. Offen bleibt das **Benachrichtigungssystem** |
